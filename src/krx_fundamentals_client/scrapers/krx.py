...
class KrxScraper(BaseScraper):
    ...
    async def fetch_listed_shares(
        ...
        있다. ``trd_dd``를 넘기면 과거 임의 거래일의 스냅샷도 받을 수 있으므로,
        point-in-time(생존편향 없는) 유니버스와 '시가총액 대비 수급 비율' 피처의
        분모를 구성할 수 있다 — **소스가 살아 있다면.**

        ⛔ **2026-08 현재 이 경로는 막혀 있다.** KRX 가 MDCSTAT 계열에 회원 로그인을
        걸면서 OTP 발급이 토큰 대신 문자열 ``LOGOUT`` 을 돌려준다. :meth:`_get_otp`
        는 비었는지와 500자 초과인지만 보므로 6자짜리 ``LOGOUT`` 이 정상 OTP 로
        통과하고, 빈 CSV 를 받아 **예외 없이 빈 dict 를 반환한다.** 즉 실패가
        "상장주식수가 없는 날" 과 구별되지 않는다.

        예전 docstring 은 "인증 없이 전 종목을 한 번에 받을 수 있다" 고 적고
        있었다 — 이 메서드를 새로 쓰는 사람이 가장 먼저 읽고 가장 크게 속는
        문장이라 남겨두지 않는다.
        """
        date = trd_dd or _recent_business_day()
        mkt_id = MARKET_CODE.get(market)
